...
const MLInsightsComponent: React.FC<MLInsightsComponentProps> = ({ 
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 p-6">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {primaryInterventions.map((intervention, index) => (
              <div key={intervention.intervention} className="relative bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl p-5 shadow-lg">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <h5 className="font-semibold text-green-900 text-lg">{intervention.label}</h5>
                  </div>
                  <div className="text-right">
                    <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-gradient-to-r ${getConfidenceColor(intervention.confidence)} text-white shadow-sm`}>
                      {(intervention.confidence * 100).toFixed(0)}%
                    </div>
                    <p className="text-xs text-green-600 mt-1">{getConfidenceLabel(intervention.confidence)}</p>
                  </div>
                </div>
      ...
      <div>
        <div className="flex items-center mb-4">
          <div className="w-6 h-6 bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center mr-3">
            <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {sortedPredictions.map((prediction) => (
            <div key={prediction.intervention} className="group relative bg-white border border-slate-200 rounded-xl p-4 hover:border-slate-300 transition-all duration-300 hover:shadow-md">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${getConfidenceColor(prediction.confidence)} text-white`}>
              ...
              <div className="flex items-center justify-between text-xs text-slate-500">
                <span className={`flex items-center ${prediction.is_predicted ? 'text-green-600' : 'text-slate-400'}`}>
                  <div className={`w-2 h-2 rounded-full mr-2 ${prediction.is_predicted ? 'bg-green-500' : 'bg-slate-300'}`}></div>
              ...
              {/* Hover Effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-indigo-500/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200/50">
        <div className="flex items-start">
          <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-lg flex items-center justify-center mr-3 flex-shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div>
            <h5 className="font-semibold text-purple-900 mb-1">Zero-Shot Classification</h5>
            <p className="text-sm text-purple-700 leading-relaxed">
              This analysis uses advanced zero-shot classification to identify evidence-based therapeutic interventions. 
              Primary interventions (highlighted in green) are high-confidence recommendations based on the patient's situation.
            </p>
          </div>
        </div>
      </div>
